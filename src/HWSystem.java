...
/**
 * Main system class managing ports and devices.
 */
public class HWSystem {
    ...
    private Device createDevice(String devName, Protocol protocol) {
        try {
            switch (devName) {
                case "DHT11": return new DHT11(protocol);
                case "BME280": return new BME280(protocol);
                case "MPU6050": return new MPU6050(protocol);
                case "GY951": return new GY951(protocol);
                case "LCD": return new LCD(protocol);
                case "OLED": return new OLED(protocol);
                case "Bluetooth": return new Bluetooth(protocol);
                case "Wifi": return new Wifi(protocol); // Fixed case
                case "PCA9685": return new PCA9685(protocol);
                case "SparkFunMD": return new SparkFunMD(protocol);
                default: System.err.println("Unknown device: " + devName); return null;
            }
    ...
    public void executeCommand(String commandLine) {
        ...
        switch (command) {
            case "list":
                if (tokens[1].equals("ports")) listPorts();
                else listDevices(tokens[1]);
                break;
            case "addDev":
    ...
    private void removeDevice(int portID) {
        ...
        String devType = device.getDevType();
        if (devType.contains("Sensor")) sensors.values().remove(device);
        else if (devType.equals("Display")) displays.values().remove(device);
        else if (devType.equals("WirelessIO")) wireless.values().remove(device);
        else if (devType.equals("MotorDriver")) motorDrivers.values().remove(device);
    ...
    private int findDevID(Device device, String devType) {
        if (devType.contains("Sensor")) {
            ...
            while (it.hasNext()) {
                ...
                if (entry.getValue() == device) return entry.getKey();
            }
        } else if (devType.equals("Display")) {
